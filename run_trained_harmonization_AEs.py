...
ap.add_argument(
    "-d", "--model_dir", type=str, default=None, help="Directory of the trained models"
)
...
if p_Method == "ASD-Standalone-AE" and p_mode == "whole":
    ...
    file_name = (
        "leave_site_out_standalone_ae_type_"
        + ae_type
        + "_method_"
        + ml_method
        + ".csv"
    ...
    for itr in range(10):
        ...
        for site in sites:
            ...
            path_to_model = os.path.join(model_dir, f'LOSO_{ae_type}_{site}.pth')
            if ae_type == "AE":   
                trained_model = AE(
                    tied=False,
                    num_inputs=num_inpp,
                ...
                trained_model = DAE(input_size=num_inpp)
                # Load the pre-trained model weights
                trained_model = torch.load(path_to_model)
                # Set the model to evaluation mode (optional, for inference)
                trained_model.eval()
            ...
            optimizer = torch.optim.Adam(trained_model.parameters(), lr=learning_rate_ae)
            # optimizer = optim.SGD(
            #     [
...
if p_Method == "ASD-Standalone-AE-10-fold" and p_mode == "whole":
    ...
    samples = pd.read_csv(PROP_SPLIT_PATH)
    ...
    for itr in range(10):
        ...
        for i in range(1):
            ...
            path_to_model = os.path.join(model_dir, f'full_model_{ae_type}_ML_RF.pth')
            if ae_type == "AE":
                trained_model = AE(
                    tied=False,
                    num_inputs=num_inpp,
